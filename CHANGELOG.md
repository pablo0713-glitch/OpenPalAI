...
- **Clearer model-failure replies** — when the model/API call fails, the agent now tells the user what actually went wrong instead of the generic "Something went sideways." `_friendly_error()` in `core/agent.py` maps the exception (by message text + HTTP status, across the Anthropic and OpenAI-compatible SDKs) to a specific reply: out of credits / billing, API-key or permission problems, rate-limiting/overload, and unreachable model service. Unknown errors still fall back to the generic message.

- **Ollama base URL ignored** — `make_adapter`/`create_adapter` built the Ollama adapter from `settings.openai_base_url` (effectively always the `http://localhost:11434/v1` default), so the configured `OLLAMA_BASE_URL` was never used. A companion on Ollama therefore failed to connect whenever Ollama wasn't reachable at the container/host's own `localhost` (e.g. an agent in a container with Ollama on the host → `APIConnectionError: Connection error`). Ollama now uses `settings.ollama_base_url`; lm_studio continues to use `settings.openai_base_url`.

- **Command Center model readout** — the Runtime sidebar's Provider/Model now reflects the **active companion's** model (`/command/status` derives it from the companion's `model_override` via the new `resolve_model()` helper, with the same `.env` fallback the runtime uses), instead of always showing the global default. Switching the Active Companion updates the readout.
